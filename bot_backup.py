...
TIMEFRAMES = ["00:05", "00:10", "00:15", "00:30", "01:00", "02:00", "05:00"]
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /start command - Login page"""
    ...
    if context.args and len(context.args) > 0:
        try:
            ...
            if referrer_id != user_id and user_info["referred_by"] is None:
                add_referral(referrer_id, user_id)
                update_user_info(user_id, **user_info)
        except ValueError:
            pass
    else:
        # Save user info even without referral
        update_user_info(user_id, **user_info)
    ...
    welcome_message = f"""
╔══════════════════════════════════╗
║  <b>🎯 𝐐𝐔𝐎𝐓𝐄𝐗 𝐒𝐈𝐆𝐍𝐀𝐋 𝐁𝐎𝐓 🎯</b>  ║
╚══════════════════════════════════╝

<b>📈 𝐖𝐞𝐥𝐜𝐨𝐦𝐞 𝐭𝐨 𝐏𝐫𝐨𝐟𝐞𝐬𝐬𝐢𝐨𝐧𝐚𝐥 𝐓𝐫𝐚𝐝𝐢𝐧𝐠 𝐒𝐢𝐠𝐧𝐚𝐥𝐬</b>
...
"""
    ...
    # For 5s, 10s, 15s, 30s: 10-20 seconds delay
    # For 1M, 2M, 5M and others: 1-3 minutes delay
    if total_seconds in [5, 10, 15, 30]:  # 5s, 10s, 15s, 30s
        trade_delay = random.randint(10, 20)  # 10-20 seconds
    elif total_seconds in [60, 120, 300]:  # 1M, 2M, 5M
        trade_delay = random.randint(60, 180)  # 1-3 minutes
    else:
        trade_delay = random.randint(60, 180)  # Default 1-3 minutes for longer timeframes
    
    # Calculate trade time with dynamic delay
    trade_time = datetime.now() + timedelta(seconds=trade_delay)
    trade_time_str = trade_time.strftime("%H:%M:%S")  # 24-hour format
    
    signal_message = f"""
🎲 <b>UTC +6:00</b>
<b>{signal['pair']} Quotex</b>
⏰ <code>{timeframe_str}</code>
🔢 <b>Sec {minutes if minutes > 0 else hours*60}</b>
...
<b>🚀 Get HIGH ACCURACY Trading Signals!</b>
...
*🚀 Get HIGH ACCURACY Trading Signals!*
...
"""
    ...
    results = [
        InlineQueryResultArticle(
            id="1",
            title="📢 Share Quotex Signal Bot",
            description="Share this amazing FREE signal bot with your friends!",
