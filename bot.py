...
async def verify_channel_membership(user_id, context, user_language_code="en"):
    """Verify if user is still in all required channels. Returns (is_member, left_channels)"""
    user_info = get_user_info(user_id)
    
    # If bot not unlocked yet, no need to check
    if not user_info.get("bot_unlocked", False):
        return True, []
    ...
    if len(not_joined) > 0:
        # User left some channels - mark bot as locked again
        user_info["bot_unlocked"] = False
        update_user_info(user_id, **user_info)
        return False, not_joined
    
    return True, []

async def send_rejoin_message(query_or_message, left_channels, user_language_code="en"):
    """Send rejoin message when user left channels"""
    is_bangla = user_language_code == "bn" or user_language_code == "bn-BD"
    
    if is_bangla:
        msg = f"""
<b>⚠️ চ্যানেল মেম্বারশিপ প্রয়োজন!</b>

আপনি নিচের চ্যানেল থেকে লিভ নিয়েছেন:
"""
...
You have left the following channel(s):
"""
        for ch in left_channels:
            msg += f"\n❌ {ch}"
        msg += "\n\n<b>Please rejoin to continue using the bot:</b>"
    
    # Create keyboard with left channels
    keyboard = []
    for channel_username in left_channels:
...
async def check_channels_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle check_channels button callback"""
    ...
    if len(not_joined) > 0:
        ...
        if is_bangla:
            msg = f"""
<b>⚠️ দয়া করে সব চ্যানেলে জয়েন করুন!</b>

আপনি এখনও <b>{len(not_joined)}</b> টি চ্যানেলে জয়েন করেননি:
"""
            for ch in not_joined:
                msg += f"\n❌ {ch}"
            msg += "\n\n<b>নিচের বাটন থেকে চ্যানেল গুলোতে জয়েন করুন</b>"
        else:
            msg = f"""
<b>⚠️ Please join all channels!</b>

You haven't joined <b>{len(not_joined)}</b> channel(s) yet:
"""
            for ch in not_joined:
                msg += f"\n❌ {ch}"
            msg += "\n\n<b>Click buttons below to join the channels</b>"
        
        # Create keyboard with only not-joined channels
        keyboard = []
        ...
        if is_bangla:
            keyboard.append([InlineKeyboardButton("✅ আবার চেক করুন", callback_data="check_channels")])
        else:
            keyboard.append([InlineKeyboardButton("✅ Check Again", callback_data="check_channels")])
        ...
        # User joined all channels - unlock bot!
        await unlock_bot_for_user(user_id, context)
        ...
        if is_bangla:
            success_msg = f"""
<b>✅ অভিনন্দন! সফলভাবে ভেরিফাই হয়েছে!</b>

🎉 আপনি এখন বট ব্যবহার করতে পারবেন!

<b>🎁 আপনার ফ্রি সিগন্যাল: {remaining_signals}</b>

<b>📊 এখন মার্কেট সিলেক্ট করুন:</b>
"""
        else:
            success_msg = f"""
<b>✅ Congratulations! Verification Successful!</b>

🎉 You can now use the bot!

<b>🎁 Your Free Signals: {remaining_signals}</b>
...
async def market_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    is_member, left_channels = await verify_channel_membership(user_id, context, user.language_code)
    if not is_member:
        await send_rejoin_message(query, left_channels, user.language_code)
        return
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif callback_data == "check_channels":
